...
class VipUserNotificationStrategyTest extends TestCase
{
    use RefreshDatabase;

    private VipUserNotificationStrategy $strategy;

    protected function setUp(): void
    {
        parent::setUp();
        $this->strategy = new VipUserNotificationStrategy;
    }

    public function test_sends_whatsapp_notification_and_logs_with_payload(): void
    {
        // Arrange
        ...
        Log::shouldReceive('info')
            ->once()
            ->with('Notificación enviada a usuario vip', Mockery::on(function ($arg) use ($user, $order, $responseBody) {
                return $arg['user_id'] === $user->id &&
                       $arg['order_id'] === $order->id &&
                       isset($arg['payload']) &&
                       is_array($arg['payload']) &&
                       $arg['payload']['type'] === 'whatsapp' &&
    ...
    public function test_handles_failed_webhook_request_with_payload_logging(): void
    {
        // Arrange
        $responseBody = '{"error": "Failed to send"}';

        Http::fake([
            'webhook.site/263d24fd-e9c9-485f-a981-9a6d0f5c95ec' => Http::response($responseBody, 500),
        ]);
